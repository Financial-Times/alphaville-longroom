...
const sendMail = (to, body) => {
	...
	return sendApi.send(to, ' FT Long Room', body).then(res => {
		if (res.ok === true) {
			return res.json();
		}
...
const sendApproved = to => {
	const emailBody = `
		<h4>You're in!</h4>
		<p>Your application to join the Long Room at FT Alphaville has been successful.</p>
	...
	return sendMail(to, htmlCompiled(model)).catch(emailErrorHandler);
};
