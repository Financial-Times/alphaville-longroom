...
module.exports = (rep) => {
	return {
		...
		countAll: () => rep.one(sql.countAll, [], a=>+a.count),
		countByTagId: (tagId) => rep.one(sql.countByTagId, {tag_id:tagId}, a=>+a.count),
		countByUserId: (userId) => rep.one(sql.countByUserId, {user_id:userId}, a=>+a.count),
