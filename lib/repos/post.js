...
module.exports = (rep) => {
	return {
		...
		selectSearch: (options = {}) => {
			return rep.any(sql.selectSearch, _.defaults({}, options, defaultOptions))
				.then(postsDbTransformation.flatten)
				.then(postsDbTransformation.enrichWithPseudonyms);
		},
		countAll: () => rep.one(sql.countAll).then(result => parseInt(result.count, 10)),
		countByTagId: (tagId) => rep.one(sql.countByTagId, {tag_id:tagId}).then(result => parseInt(result.count, 10)),
		countByUserId: (options = {}) => rep.one(sql.countByUserId, _.defaults({}, options, defaultOptions)).then(result => parseInt(result.count, 10)),
		countSearch: (options = {}) => rep.one(sql.countSearch, _.defaults({}, options, defaultOptions)).then(result => parseInt(result.count, 10)),
