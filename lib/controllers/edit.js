			...
			.then(originalPost => {
				...
				if (!req.userData || (req.userData.user_id !== originalPost.user_id && !req.userData.is_editor)) {
					...
					if (Object.keys(validation).length) {
						...
						const getNewImageEmbedQueries = function (transaction) {
						...
						const getDeletedImageEmbedQueries = function (transaction) {
							const oldSummary = originalPost.summary;
							const newSummary = req.body.summary.trim();

							if (oldSummary) {
								const filesToRemove = [];

								const images = oldSummary.match(/src\=\"[^"]+\/files\/([^"]+)\"/g) || [];

								return Promise.all(images.map(matchedString => {
									if (!newSummary || newSummary.indexOf(matchedString) === -1) {
										const fileNameMatch = matchedString.match(/src\=\"[^"]+\/files\/([^"]+)\"/);
						...
						return db.tx(transaction => {
							return Promise.all([
								...
								getNewImageEmbedQueries(transaction),
								getDeletedImageEmbedQueries(transaction)
							]).then(queries => {
