...
const UserStatusError = (userStatus) => {
	...
	db.user.find(userUuid)
		.then(user => {
			if (user && (user.status === 'pending' || user.status === 'revoked' || user.status === 'approved')) {
				throw UserStatusError(user.status);
			}
		...
		.catch((err) => {
			if (err instanceof (UserStatusError)) {
				...
				if (err.status === 'revoked') {
					return res.render('nonMemberMessage', {
						message: 'Your access has been suspended. Any question please email us at <a href="mailto:alphaville@ft.com>alphaville@ft.com</a>'
					});
				}
