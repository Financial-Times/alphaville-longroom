...
module.exports = function (options) {
	...
	const alphavilleHbs = exphbs.create({
		defaultLayout: path.join(
			...
			"node_modules/alphaville-ui/layout"
		),
		extname: ".handlebars",
		partialsDir: [
			path.join(options.directory, "views", "partials"),
			path.join(options.directory, "bower_components"),
			path.join(options.directory, "node_modules"), // new
			// path.join(options.directory, "node_modules/@financial-times"), // new
			// path.join(options.directory, "node_modules/"), // new
		],
		helpers: {
			block: function (name) {
	...
	app.get(
		...
		`/assets/${options.appBasePath}/:fingerprint/*.(woff|svg|ttf|eot|gif|png|jpg|js|css)`, // new
		(req, res) => {
			...
			res.sendFile(path.join(options.directory, "/node_modules", newPath)); // new
		}
	);
	app.use(
